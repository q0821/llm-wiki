...
## [2026-07-16] ingest | 5 步驟建 Fable 5 個人 AI OS（等號 Substack）
- 來源：https://denghao.substack.com/p/5-fable-5-ai-os （markdown.new API 限流，改 WebFetch 抓全文回填 raw/）
- 分類：論點型／方法框架 → 加個人吸收段；信心中（框架清晰、真實開源元件，但單一教學來源無一手驗證）
- 建立來源頁：[[src-denghao-fable5-ai-os-2026-07]]（5 步驟：大腦 SKILL.md／記憶 Obsidian／聲音本地 STT-路由-TTS／介面 HUD／交付換皮）
- **並列標註內部矛盾**（per CLAUDE.md 矛盾不擅自取捨）：大腦是 Fable 5 雲端模型、第四步串 YouTube/IG API、路由可用輕量 API → 「100% 本地」只對語音 I/O 成立、非端到端；「換皮轉賣」低估產品化難度
- 跨頁連結：[[src-voice-typing-mac-github-2026-06]]（第三步現成替代）、[[comparison-stt-models-whisper-paraformer-sensevoice]]（自建 STT 選型）、[[Obsidian]]、[[LLM-Wiki]]、[[HermesOS]]/[[Hermes-Agent]]、[[Agent-Skills]]
- 未建額外 entity/concept 頁（避免與既有 AI OS 生態頁冗餘）
- 影響頁面：[[src-denghao-fable5-ai-os-2026-07]], index.md

## [2026-07-16] reflect | 個人吸收第 15 次 — 新 taxonomy 候選「手段過重錯位／自建 vs 現成」
- 對應來源：[[src-denghao-fable5-ai-os-2026-07]]
- Quiz：Q1「用語音來控制跟回應，蠻有趣的」（＝第三步 The Voice）/ Q2「想用在自己工作上，因人因因素想用語音操控降低打字負擔」
- **錯位觀察**：場合是真實剛需（人因／身體負擔，非純收藏），方向對（語音確能解痛點），但**手段規模嚴重超標**——需求是「今天少打點字」，來源卻是「從零 DIY 一整套可轉賣 AI OS」，語音只是五零件之一且為自建管線版
- **新 taxonomy 候選「手段過重錯位／自建 vs 現成」**：有別於尺度錯位、階段窄化錯位；使用者被重型自建方案吸引（因「酷炫有趣」），真實需求用輕型現成工具即滿足；愈有真實痛點愈易被拐走 → 掉過度工程兔子洞，痛沒解先花數週寫 code。危險性高
- **重組路徑**：立刻用現成（macOS 語音控制/聽寫、或 wiki 內既有 [[src-voice-typing-mac-github-2026-06]]）解痛優先；文章唯一可日後抽出的是第三步「意圖路由器」概念（第二階段才需要）；明確不要做第 1/2/4/5 步（那是做產品路線，與減少打字無關）
- Spaced retrieval 約定：~2026-07-30（14 天後），**行動確認**式回測「有沒有真的開始用語音輸入？打字負擔降了嗎？還在糾結自建就回想：先用現成、解痛第一」
- 對應 [[Ratchet-Pattern]]

## [2026-07-16] ingest | 投資三堂課 心法／實戰／風控（孟恭，inbox 轉貼）
- 來源：`投資觀念.md`（inbox 轉貼，非使用者原創；使用者確認僅轉存）
- 分類：論點型（投資心法）→ 加個人吸收段；信心中（講者身份推測＝股癌謝孟恭、二手整理稿、心法難證偽）
- 建立來源頁：[[src-invest-3-lessons-menggong-2026-07]]（三堂課：心法灰階思考/實戰全資收購/風控不死三鐵律）
- **標明分界**：檔案內「我的體會」為原整理者觀點，非使用者本人 → 另段收錄並標註
- wiki **第一個投資「心法/框架」層**來源（既有 [[雙幣贏]]/[[加密貨幣收益工具比較]] 等皆工具戰術層）
- 跨頁連結：[[決策疲勞]]（寫下決策當反方攻擊＝同一招）、[[src-taiwan-stock-2026-03-09]]/[[src-btc-dual-currency-2026-03-09]]（可用三鐵律回檢實際部位）
- 影響頁面：[[src-invest-3-lessons-menggong-2026-07]], index.md

## [2026-07-16] reflect | 個人吸收第 16 次 — 新錯位「來源歸屬錯位」
- 對應來源：[[src-invest-3-lessons-menggong-2026-07]]
- Quiz：Q1「停損停利別人的方法，可以適當減壓？」/ Q2「確保自己不會一次風險就倒掉」
- **Q2 對位極佳**：正是第三課「不死」核心（分散/不 All-in/不槓桿/單一標的上限）
- **新錯位類型「來源歸屬錯位」**：Q1 把重點記成「用**別人的**方法」，恰與第一課核心「不跟任何人（含講者）、看原廠不看副廠、自訂規則」相反——把這篇最強調的「獨立判斷」記反了來源歸屬。實質風險：照「用別人方法」易變找明牌/跟單，正是講者要戒的
- 文中停損停利本質＝「買進理由消失就賣」「跌破支撐減碼」的**自訂規則式紀律**，非套用他人方法
- **重組路徑**：Q1 校正為「停損停利規則要自己事先訂（買進理由/減碼點位），照規則執行」；Q2 落地＝拿三鐵律當 checklist 回檢 [[src-taiwan-stock-2026-03-09]] 類實際部位
- Spaced retrieval 約定：~2026-07-30，回測「停損停利規則該誰訂？」期待答「我自己事先訂」確認錯位校正 + 有沒有把某部位套『單一標的上限＋不 All-in』檢查
- 對應 [[Ratchet-Pattern]]

## [2026-07-16] ingest | 為自己建一套 Skill／Agent 體系（未署名，inbox 轉貼）
- 來源：`未命名-20260716-001519.md`（inbox 轉貼，未署名 FB/部落格論述）；使用者狀態：未讀、看到有興趣先存
- 分類：論點型（AI 趨勢）→ 加個人吸收段；信心中（未署名個人觀點、無數據）
- 建立來源頁：[[src-build-your-own-agent-system-2026-07]]
- **本篇幾乎是 wiki 既有主題匯流點**：[[決策疲勞]]（省的是判斷不是打字＝同一命題）、[[多模型角色編排]]/[[src-zhongyea-orchestrator-cost]]/[[src-nanako-pilotfish]]（Anthropic 管理者框架 Fable 5+Sonnet）、[[Agent-Skills]]、[[Harness-Engineering]]、[[LLM-Wiki]]（放 Git+記改動+最小發佈＝使用者本 wiki 在做的事）
- 細節：作者推薦用 GitHub 而非筆記軟體管理 Skill（文中「Hyday」疑為 Hi Day 筆誤 → [[src-muki-plaud-workshop-ai-codev-2026-06-04]]）
- 影響頁面：[[src-build-your-own-agent-system-2026-07]], index.md

## [2026-07-16] reflect | 個人吸收第 17 次 — 未讀+收藏，預先標記「對你邊際價值低、真正場合是對外科普素材」
- 對應來源：[[src-build-your-own-agent-system-2026-07]]
- Quiz：Q1「完全忘記」/ Q2「看到有興趣先存、還沒細讀」
- **預先標記價值（非測回憶）**：這篇每一條使用者早在做或早已 ingest（本 wiki＝Skill/Agent 體系、[[決策疲勞]]、多模型編排群）→ 對使用者是「既有認知的漂亮複述」，新知邊際價值低
- **真正再利用場合**：把「你已在做的事講成好懂科普」，是對客戶/同事解釋「為何建自己 Skill 體系、為何非模型愈聰明愈好」的現成溝通素材 → 當「待用素材」歸檔，不是「待學習」
- 未讀風險≈0（不讀不會錯過），不急讀；真讀帶「哪句能拿去跟客戶講」的目的掃過即可
- Spaced retrieval 約定：~2026-07-30，行動確認「有沒有翻過？有沒有撈到一句能解釋『卷協作 vs 卷聰明』的話？沒翻也 OK＝確認只是複述已知」
- 對應 [[Ratchet-Pattern]]

## [2026-07-16] ingest | 八個生活提醒 Mel Robbins Let Them（inbox 轉貼）
- 來源：`未命名 1-20260716-001526.md`（inbox 轉貼，某整理者對 Mel Robbins 影片重點整理）；使用者狀態：未讀、要求 LLM 導讀
- 分類：收藏型偏自我成長（非工作論點，正常不強制吸收段；因使用者主動要求導讀＝有吸收意圖，保留精簡吸收段）
- 建立來源頁：[[src-mel-robbins-8-life-reminders-2026-07]]（主框架 Let Them Theory 由他們去＋Let Me；8 條情緒界限提醒）
- 跨頁連結：[[決策疲勞]]（弱連結：第 2 條「值不值得消耗精力」＝情緒版減少重複決策）
- 影響頁面：[[src-mel-robbins-8-life-reminders-2026-07]], index.md

## [2026-07-16] reflect | 個人吸收第 18 次 — 收藏型自我成長，LLM 導讀挑對症條目
- 對應來源：[[src-mel-robbins-8-life-reminders-2026-07]]
- 使用者未讀、主動要求導讀 → 不考回憶，改 LLM 讀完挑對「接案/經營者、易內耗」最對症的條目
- **挑重點**：第 2（暫停鍵：值不值得消耗精力）、第 3+7（別人情緒是資訊不是指令／界限是留給自己的）最對症客戶情緒場景；第 8 收尾行動法；其餘通用勵志不強記
- **一句話收斂**：Let Them（不歸你管的別扛）＋ Let Me（只管你能控制的：態度/努力/選擇/時間/精力）
