...
type: source
sources: ["Nanako0129pilotfish Multi-model orchestration layer for Claude Code.md"]
created: 2026-07-11
updated: 2026-07-11
...
**授權**：MIT
**數據點（時間敏感）**：319 stars、24 forks、最新 v1.1.2（2026-07-10）
**抓取於**：2026-07-11
...
2. **角色層**（六個 agent 檔）：每個角色用 frontmatter 綁定適當模型層級
3. **政策層**（[[CLAUDE-md|CLAUDE.md]]）：用**角色名**定義下放規則，**絕不寫模型名**

## 六角色（詳見 [[多模型角色編排]]）

- `scout`（Haiku）唯讀查找／`Explore`（Haiku）覆蓋內建 Explore agent
- `mech-executor`（Sonnet）機械式模式工作
- `executor`（Opus）需判斷的實作／`verifier`（Opus）fresh-context 對抗式 review／`security-executor`（Opus）資安
