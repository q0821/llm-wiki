...
| 頁面 | 摘要 |
|------|------|
| [[src-taony-ai-ecommerce-page-uiux-2026-08]] | **新來源（論點型／YouTube 公開描述）**：Taony Design 拆解 AI 一小時完成的電商頁面為何「合格但不一定好」。四個判斷點：先給產品脈絡、方向錯就重設範圍、回饋要寫修改理由、兩輪仍不對就進 Figma。影片無字幕，頁面只保存公開描述與章節，未擅自補寫三個版本的視覺差異。**個人吸收**：使用者抓到「修改要說明為什麼」，應用於從零開始的 AI 設計；重組為「變更 + 原因 + 預期效果」回饋路徑 |
| [[DESIGN-md]]（更新） | 補充沒有完整設計系統時，以產品脈絡與具理由的回饋約束 AI，兩輪仍偏離再進 Figma 的替代路徑 |
| [[impeccable]]（更新） | 補充視覺反模式檢查不能取代產品脈絡、方向判斷與設計師人工介入 |
| [[Figma]]（更新） | 補充 AI 連續兩輪仍未對準時，由設計師手動調整並記錄設計意圖的角色 |
| [[Claude Code]]（更新） | 補充 Claude Code 在 AI 設計流程中接收產品脈絡、依理由迭代，必要時交由 Figma 收斂 |
| [[Agentic-AI-Workflow]]（更新） | 以實作觀察補強 Reviewer 的具體操作門檻與從生成到人工收斂的流程 |
...
| 頁面 | 摘要 |
|------|------|
| [[src-mel-robbins-8-life-reminders-2026-07]] | **新來源（收藏型／自我成長）**：Mel Robbins（梅爾羅賓斯）影片「八個生活提醒」，主框架 **Let Them Theory（由他們去）＋ Let Me（只管你能控制的）**。8 條：珍惜當下(95 歲視角)、反應前按暫停鍵、別人情緒是資訊不是指令、別等感覺好才行動、為自己幸福負責、改變未來、界限是留給自己的、把今天當可創造的平凡星期二。`#好適合容易內耗的人`。使用者未讀、要求 LLM 導讀 → 挑對接案者最對症的第 2/3/7 條，導向 Let Them＋Let Me 一句話 |
| [[src-build-your-own-agent-system-2026-07]] | **新來源（論點型／AI 趨勢，未署名）**：「今年關鍵不是哪家模型最聰明，而是你有沒有建自己的 Agent 體系」。從會用介面→擁有 AI 團隊；三層理由(現成 agent 不懂你語境／省的是判斷不是打字／模型差距縮小時誰更會設計 agent 放大差距)；Skill→Agent→系統(放 Git、記改動、最小發佈流程)。**幾乎是 wiki 既有主題匯流點**（[[決策疲勞]]／[[多模型角色編排]]／[[Agent-Skills]]／[[Harness-Engineering]]）。個人吸收：未讀+收藏，**預先標記對使用者新知價值低（早在做）、真正場合是對客戶科普 AI 協作的現成素材** |
| [[src-invest-3-lessons-menggong-2026-07]] | **新來源（論點型，投資心法）**：孟恭（研判股癌謝孟恭，信心中）三堂課——心法(灰階思考/不跟任何人含講者自己/看原廠不看副廠)、實戰(全資收購心態選股/大跌買指數大型權值)、風控(不死哲學/三鐵律：分散、不 All-in、不碰槓桿)。wiki **第一個投資「心法/框架」層**來源（既有投資頁全是工具戰術層）。原整理者「我的體會」另收但標明非使用者觀點。**個人吸收（新錯位「來源歸屬錯位」）**：Q2「不會一次倒掉」對位極佳＝第三課核心；Q1 把重點記成「用**別人的**停損停利方法」，恰與第一課「不跟任何人、自訂規則」相反＝把獨立判斷記反來源，有跟單風險，導向「規則要自己訂」 |
| [[src-denghao-fable5-ai-os-2026-07]] | **新來源（論點型／方法框架）**：等號 Substack「5 步驟建 Fable 5 個人 AI OS」——大腦(技能架構/SKILL.md)＋記憶([[Obsidian]])＋聲音(本地 STT faster-whisper／意圖路由／TTS Kokoro)＋介面(HUD 儀表板)＋交付(打包換皮轉賣)。**並列標註內部矛盾**：大腦是 Fable 5 雲端模型、第四步串 YouTube/IG API，故「100% 本地」只對語音 I/O 成立非端到端；「換皮轉賣」低估產品化難度。信心中（框架清晰、單一教學來源無一手驗證）。**個人吸收（新 taxonomy 候選「手段過重錯位／自建 vs 現成」）**：使用者想用語音減少打字（真實剛需，人因因素），但被「從零 DIY 整套 AI OS」框架吸引＝方向對、手段嚴重超標；快路已在自己 wiki（[[src-voice-typing-mac-github-2026-06]]／macOS 內建語音控制），導向先用現成、解痛優先 |
...
| [[src-denghao-fable5-ai-os-2026-07\|5 步驟建 Fable 5 個人 AI OS（等號）]] | 論點型／方法框架：大腦(SKILL.md 技能架構)＋記憶(Obsidian)＋聲音(本地 STT/路由/TTS)＋介面(HUD)＋交付(打包換皮轉賣)；並列標「100% 本地」與 Fable 5 雲端大腦/API 串接的矛盾；信心中；含個人吸收（手段過重錯位：想用語音減打字→導向現成工具而非自建 OS） | 1 |
