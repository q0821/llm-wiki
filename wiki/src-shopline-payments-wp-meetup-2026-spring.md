...
**4 次後的觀察更新**：對位 fit 度逐次提升（完全錯位 → 層級錯位 → 尺度錯位 → 階段窄化錯位）。可能的規律：
- 來源**本身就是 framework** 時（雷小蒙的雙 AI 對辯 / 歐陽嘉隆的 5 工具串接 / SHOPLINE 的 3 殺手），對位比較容易
- 來源是**單點 insight** 時（曹興誠的 CMOS 決策），對位最容易完全偏掉
- **依然存在的模式**：使用者記住的是「**單點/單一 framework**」，但實際應用場合的範圍**幾乎都比工具的 cover 範圍大**
