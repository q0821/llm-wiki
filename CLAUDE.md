...
### Ingest 個人吸收段（2026-05-28 確立）

對「論點型」來源，ingest 時必須：
1. **建頁前先 quiz 使用者**：「不看文章用自己的話講 1 個重點」+「打算用在什麼場合」
2. **觀察論點/場合錯位**：使用者記得的論點是否真的能用在他打算的場合？若不對位，明確指出
3. **寫進 src 頁最末「個人吸收」段**：含使用者答案 + LLM 觀察 + 重組路徑建議 + spaced retrieval 約定（7-14 天後回測）

來源分類判斷：

| 類型 | 範例 | 個人吸收段 |
|------|------|----------|
| **論點型** | 觀點論述、框架、理論、方法、決策模型、商業評論 | **加** |
| 採購/清單型 | 品牌清單、工具清單、購物參考 | 不加（用到時 query 即可）|
| 備忘型 | 個人事件紀錄、硬體故障、生活感想 | 不加 |
| 官方文件型 | 產品落地頁、API 文檔、合約條文 | 不加（除非含關鍵論述）|

不確定時主動問使用者。動機：使用者反映「存很多但沒好好吸收」，主動加吸收檢核而非被動等使用者自查。對應 [[Ratchet-Pattern]]。
...
- **ingest 完不主動建社群素材**：不要在 ingest 流程順手產出社群貼文 / 知識卡片（`~/WORK/case/社群貼文/`）。只在使用者明確要求時、且只對指名的那篇才建。發布前的設計方向規則見該資料夾自己的 CLAUDE.md。
